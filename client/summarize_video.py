...
events described by a Vision Language Model (VLM).
The log contains descriptions of individual frames from a video stream, including timestamps and VLM 
observations.
Your goal is to provide a concise, coherent, and chronological summary of what occurred across all frames.
Focus on identifying key actions, changes in the scene, and any significant objects or interactions.
Do not invent information. If an action is repeated, note its duration or recurrence.
Your summary should be presented as a flowing narrative, connecting the observations naturally.
You should be as short and concise as possible within 400 characters more or less
"""

# The user query will implicitly be the content of the VLM output file.
...
def main():
    ...
    payload = {
        ...
        "user_query": vlm_output_content # The entire VLM output file content
    }

    start_time = time.time()
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Sending VLM output for summarization...")
    print(f"  To Endpoint: {SERVER_ENDPOINT}")
    print(f"  Using System Prompt: {SUMMARIZATION_SYSTEM_PROMPT[:100]}...") # Print a truncated version

    try:
        # Send the request to the new summarization endpoint
...
seconds.""")
        print("""The summarization model (llama3-70B-cool:latest) might be very slow or requires more 
resources.""")
    except requests.exceptions.ConnectionError as e:
        print(f"""[{datetime.now().strftime('%H:%M:%S')}] Connection error: {e}. Is the server running and 
accessible?""")
    except requests.exceptions.RequestException as e:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] An unexpected request error occurred: {e}")
    except json.JSONDecodeError:
        print(f"""[{datetime.now().strftime('%H:%M:%S')}] Error decoding JSON response from server. Raw 
text: {response.text}""")
    except Exception as e:
